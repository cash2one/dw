...
def convertOneLine(line, current_index):
    ...
    pos = param_name.find('E_');
    if( pos >= 0):
        param_name = param_name[pos+2:]
    #print param_name
    pos = param_name.find('pcode_');
    if( pos >= 0):
        param_name = param_name[pos+6:]
...
#            param_notes.append(note)
    ...
    for i in range(0, len(param_names) - 1):
        ...
        if len(param_names[i]) == 0:
            pass
        elif int(param_values[i]) >= 1000: 
			file_handle.write('\t<message opcode=\'' + param_values[i] + '\' name=\'' + param_names[i] + '\' />\n')
...
def convertEnum(file_name, output_path):
    try:
        ...
        f_write.write('<?xml version="1.0" encoding="utf-8" ?>\n')
        f_write.write('<root>\n')
