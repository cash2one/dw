...
{
	BATTLE_LOG_INFO("ManagerSession connection new : <%x>", session_id);
...
void ManagerClientSession::handleOutputPacket(InterPacket * pack)
{
	handleOutputStream(pack->session_id, pack->getStream(), pack->getStreamSize());
	pack->cleanStream();
	delete pack;
}
