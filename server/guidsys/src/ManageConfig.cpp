...
ManageConfig * ManageConfig::m_instance = NULL;
...
bool ManageConfig::loadConfig(const string & cfg_path)
{
	...
	if (m_config_doc->load_file(cfg_path))
	{
		...
		if (NULL != root_ele)
		{
			...
			Attribute * port_attr = root_ele->get_attribute("port");

			if (NULL == m_seed_attr || NULL == m_index_attr || NULL == addr_attr || NULL == port_attr)
			{
				return false;
			}
			 ...
			 m_guid_cfg.port = port_attr->as_int();
