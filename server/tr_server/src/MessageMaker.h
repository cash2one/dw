...
public:
	...
	static InterPacket * makeSMsgNotifyBattleMatchResult(SessionId session_id, uint64_t match_battle_id, MatchCellVec_t & match_cell_vec);

	static InterPacket * makeMsgMatchAck(SessionId session_id, uint64_t account_id, int result);

	static InterPacket * makeMsgCancelMatchAck(SessionId session_id, uint64_t account_id, int result);
};
