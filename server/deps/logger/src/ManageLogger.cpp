...
ManageLogger * ManageLogger::m_instance = NULL;
...
int ManageLogger::svc()
{
	...
	while (!m_stop)
	{
		...
		{
			boost::mutex::scoped_lock lock(m_logger_mutex);
			int num = m_logger_ctn.size();
			if (num > 0)
			{
		...
		{
			{
				boost::mutex::scoped_lock lock(m_logger_flush_set_mutex);
...
int ManageLogger::init()
{
	...
	boost::function0<int> f = boost::bind(&ManageLogger::svc, this);
