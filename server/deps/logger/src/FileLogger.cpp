...
string FileLogger::makeLogFileName(const string & file_path)
{
	...
	//boost::gregorian::date tv(boost::gregorian::day_clock::local_day());
	boost::posix_time::ptime tv(boost::posix_time::second_clock::local_time());

	int month_value = tv.date().month();
	int day_value = tv.date().day();

	sprintf((char *)result.data(), "%s_%4d-%02d-%02dT%02d_%02d_%02d", file_path.c_str(), tv.date().year(), month_value, day_value, tv.time_of_day().hours(), tv.time_of_day().minutes(), tv.time_of_day().seconds());
