...
uint32	Logger::m_filter_mask = LL_DEBUG;
int		Logger::m_buffer_length = 1024;

time_t getSystemMilliSeconds()
{
...
void formatTimeToString(time_t time_in_millsec, string & formated_time)
{
	boost::posix_time::ptime time_value = boost::posix_time::from_time_t(time_in_millsec / 1000);
	formated_time = boost::posix_time::to_iso_string(time_value);
}

time_t formatStringToTime(const string & str_time)
{
	boost::posix_time::ptime time_value = boost::posix_time::from_iso_string(str_time);
	time_t tv = boost::posix_time::to_time_t(time_value);
	return tv * 1000;
}
...
//void Logger::setPrefix(const string & module_name, const string & class_name)
//{
...
void Logger::log(LOGGER_LEVEL log_level, const char * log_format, ...)
{
	...
	boost::posix_time::ptime tv(boost::posix_time::microsec_clock::local_time());

	auto date_value = tv.date();
	auto time_value = tv.time_of_day();

	int day_value = date_value.day();

	sprintf((char *)str_date.data(), "%4d-%02d-%02dT%02d:%02d:%02d.%03d", date_value.year(), date_value.month(), day_value, time_value.hours(), time_value.minutes(), time_value.seconds(), time_value.total_milliseconds() % 1000);
...
{
	{
		boost::mutex::scoped_lock lock(m_sync_logger_mutex);
