...
int DynamicParser::init(const string proto_path)
{	
	std::cout << "DynamicParse::init, path: " << proto_path << std::endl;
	...
	try
	{
		...
		{
			std::cout << "DynamicParse::init recursive path: " << beg_iter->path().filename().generic_string() << std::endl;
...
// 			
	...
	{	
		std::cout << "DynamicParse::init filesystem_error: " << e.what() << std::endl;
		return -1;
	}
	catch (const std::exception& e)
	{
		std::cout << "DynamicParse::init exception: " << e.what() << std::endl;
		return -1;
	}
...
void DynamicParser::ReadDesAndInsert(const google::protobuf::Message& message, MessageMap*& map_content)
{
	...
	for (int i=0; i<count; ++i)
	{
			...
			{	
				...
				else
				{
					...
					case google::protobuf::FieldDescriptor::TYPE_INT32:
						value = Convert::toStringA(refl->GetRepeatedInt32(message, field_des, j));
						break;
					case google::protobuf::FieldDescriptor::TYPE_UINT32:
						value = Convert::toStringA(refl->GetRepeatedUInt32(message, field_des, j));
						break;
					case google::protobuf::FieldDescriptor::TYPE_INT64:
						value = Convert::toStringA(refl->GetRepeatedInt64(message, field_des, j));
						break;
					case google::protobuf::FieldDescriptor::TYPE_UINT64:
						value = Convert::toStringA(refl->GetRepeatedUInt64(message, field_des, j));
						break;
					default:
						break;
					}
		...
		else
		{
			...
			else
			{
				...
				{
				case google::protobuf::FieldDescriptor::TYPE_INT32:
					value = Convert::toStringA(refl->GetInt32(message, field_des));
					break;
				case google::protobuf::FieldDescriptor::TYPE_UINT32:
					value = Convert::toStringA(refl->GetUInt32(message, field_des));
					break;
				case google::protobuf::FieldDescriptor::TYPE_INT64:
					value = Convert::toStringA(refl->GetInt64(message, field_des));
					break;
				case google::protobuf::FieldDescriptor::TYPE_UINT64:
					value = Convert::toStringA(refl->GetUInt64(message, field_des));
					break;
				default:
					break;
				}
...
google::protobuf::Message* DynamicParser::createMessage(const std::string& typeName)
{	
	// 从 生成的cpp 代码里 new
	using namespace google::protobuf;
	const DescriptorPool * des_pool = DescriptorPool::generated_pool();
...
const google::protobuf::Message* DynamicParser::GetPrototype(const std::string& type_name)
{
...
void DynamicParser::delMsgDesc(MsgVec & vec)
{
...
int DynamicParser::checkEntity(const TableColumnsMap & a_outcols, const std::string& a_db_name, std::vector<std::string>& a_changed)
{
	std::stringstream fn;
	...
	for (auto itr = m_protofiles.begin(); itr != m_protofiles.end(); ++itr)
	{
		...
		for (int i = 0; i < msg_count; ++i)
		{
			...
			if (NULL == descriptor)
			{
				DEF_LOG_ERROR("not found message<%d> in %s ", i, protofile.c_str());
				continue;
			}
...
int DynamicParser::updatedbChanged(const std::string& a_ip, uint32 a_port, const std::string& a_user, const std::string a_pwd,
...
{	
	...
	try {
		...
		DEF_LOG_ERROR("DB connection failed:%s\n, db name:%s, ip:%s, port:%u, user:%s, pwd:%s",
...
int DynamicParser::querydbDesc(const std::string& a_ip, uint32 a_port, const std::string& a_user, const std::string a_pwd,
...
{	
	...
	{
		DEF_LOG_ERROR("DB connection failed, db name:%s, ip:%s, port:%u, user:%s, pwd:%s",
	...
	{
		std::string table_name = table_res.at(i).at(0).c_str();
		...
		{
			const std::string column_name = columns_res.at(j).at(0).c_str();
...
int DynamicParser::getMsgDesc(const std::string& type_name, MsgVec & vec)
{
...
int DynamicParser::getMessageContent(const std::string& type_name, const char* buf, int buf_size, MessageMap*& map_content)
{
	google::protobuf::Message* msg = createMessage(type_name);
	if (msg == NULL)
		return -1;
	
	if (!msg->ParseFromArray(buf, buf_size))
		return -1;
...
void DynamicParser::deleteMessageContent(MessageMap*& map_content)
{
	if (map_content == NULL)
	{
		return;
...
void DynamicParser::deleteMsg(MessageMap*& map_content)
{
...
{	
	...
	try 
	{
		...
		query << " FROM " << removeNamespace(msg->GetTypeName());
		query << " WHERE guid=" << myguid;
		const ::mysqlpp::StoreQueryResult& res = query.store();
		if (res.size() == 0) {
			er_code_ = CAE_NOT_FOUND;
			er_str_ = "Cannot find " + removeNamespace(msg->GetTypeName()) + "<<" + ::boost::lexical_cast<::std::string>(myguid)+">>";
			return false;
		}
...
bool CacheAssistantx::update(void* q)
{
	...
	try {
		query.clear();
		query << "UPDATE ";
		query << removeNamespace(msg->GetTypeName());
		query << " SET ";
...
{	
	...
	try {
		query.clear();
		query << "INSERT INTO ";
		std::string temp = removeNamespace(msg->GetTypeName());
		query << removeNamespace(msg->GetTypeName());
		query << " SET ";
...
CacheAssistantx* DynamicParser::create(uint64 guid, const std::string name)
{
...
::google::protobuf::Message* DynamicParser::create(const std::string name)
{
