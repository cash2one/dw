...
InterPacket * MessageMaker::makeMsgMatchEnterBattle(MyGuid_t user_id, SessionId session_id, const string & server_ip, int server_port)
{
	...
	pack->setMsgInfo(Opcode_E_MsgMatchEnterBattle, user_id, builder.GetSize(), (char *)builder.GetBufferPointer());
	pack->session_id = session_id;
	return pack;
}

InterPacket * MessageMaker::makeSMsgNotifyBattleMatchResult(SessionId session_id, MatchCellVec_t & match_cell_vec)
{
	FlatBufferBuilder builder;
	...
	Offset<SMsgNotifyBattleMatchResult> off_match_result_set = CreateSMsgNotifyBattleMatchResult(builder, 1, 1, off_pi);
...
InterPacket * MessageMaker::makeMsgMatchAck(SessionId session_id, uint64_t account_id, int result)
{
	...
	InterPacket * pack = new InterPacket();
	pack->setMsgInfo(Opcode_E_MsgMatchAck, account_id, builder.GetSize(), (char *)builder.GetBufferPointer());
	pack->session_id = session_id;
